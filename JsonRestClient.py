...
# The URL's that will be used for the GET and POST request need to be added to the variables below,
# they are of the form "https://<URL>/get?userKey=<USER_AUTH_TOKEN>" for the GET request and,
# "https://<URL>/post?userKey=<USER_AUTH_TOKEN>" for the POST request
GET_URL = '<INSERT_GET_API_URL_HERE>'
POST_URL = '<INSERT_POST_API_URL_HERE>'
...
def process_json_data(json_data):
    ...
    # Get the unique list of countries and available dates for those countries, this will help to build
    # a list of unique countries for the response
...
if __name__ == '__main__':
    ...
    get_status_code = get_request(GET_URL)
    if get_status_code == HTTP_STATUS_OK:
        print("Okay GET worked, time to POST")
        post_status_code = post_request(POST_URL)
        if post_status_code == HTTP_STATUS_OK:
            print("Woohooo!!")
        else:
            print("Woops! Looks like something went wrong with the POST call, the return code was %s" % post_status_code)
    else:
        print("Woops! Looks like something went wrong with the GET call, the return code was %s." % get_status_code)